...
export default function Agents() {
  ...
  const renderPotential = () => (
    <div className="lonko-potential">
      ...
      <section className="card lonko-daily-status"><div><span className={`status-badge status-${dailyStatus?.installed && dailyStatus?.vault?.available && dailyStatus?.telegram?.configured ? 'ok' : 'warn'}`}>{dailyStatus?.installed ? 'AUTOMATIZACIÓN ACTIVA' : 'NO INSTALADA'}</span><h3>Ciclo diario semiautónomo</h3><p>Especialista rotativo → auditoría WEICHAFE → consolidación LONKO → Telegram y Alexa segura.</p></div><div className="lonko-daily-metrics"><span><small>Horario</small>{dailyStatus?.schedule?.time || '20:00'} · Santiago</span><span><small>Obsidian</small>{dailyStatus?.vault?.latestDaily?.name || 'Sin reporte'}</span><span><small>Telegram</small>{dailyStatus?.telegram?.configured ? 'Resumen diario' : 'No configurado'}</span><span><small>Alexa · AYLÉN</small>{dailyStatus?.alexa?.installed ? dailyStatus?.alexa?.presenceVerified ? 'Lista' : 'Espera presencia' : 'No configurada'}</span></div></section>
